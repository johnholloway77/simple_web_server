...
main(int argc, char *argv[])
{
  ...
  if(!(app_flags & D_FLAG))
    {
      /*
       * We are setting nochdir to -1 so that the daemon runs in the current
       * working directory. Otherwise entering the correct url for a page is a
       * pain.
       *
       * nullfd is set to 1 to direct stdin, stdout and stderr output do
       * /dev/null
       */
      daemon(-1, 0);
    }
